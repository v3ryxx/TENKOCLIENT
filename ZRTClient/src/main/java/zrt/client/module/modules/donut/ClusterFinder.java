...
public class ClusterFinder extends Module {
    ...
    private final ScheduledExecutorService exec = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t=new Thread(r,"ZRT-ClusterFinder");t.setDaemon(true);t.setPriority(3);return t;
    });
    ...
    private void scan() {
        ...
        try {
            ...
            for(Map.Entry<BlockPos,Integer> e:hot.entrySet())
                if(e.getValue()>=min){BlockPos k=e.getKey();found.add(new Cluster(new Vec3d(k.getX()+4,k.getY()+4,k.getZ()+4),e.getValue()));}
    ...
    private void render(Events.RenderWorldEvent e) {
        ...
        for(Cluster c:results.get()){
            Color col=new Color(180,0,255,180);
            if(showLines.get()) RenderUtil.drawLineToPos(e.matrices,mc.player,c.center,col);
            RenderUtil.drawLabel(e.matrices,c.center,"§5Cluster §7x"+c.count,e.tickDelta);
        }
    }

    public static class Cluster{public final Vec3d center;public final int count;public Cluster(Vec3d c,int n){center=c;count=n;}}
}
