...
public class LightFinder extends Module {
    ...
    private final ScheduledExecutorService exec = Executors.newSingleThreadScheduledExecutor(r->{Thread t=new Thread(r,"ZRT-LightFinder");t.setDaemon(true);t.setPriority(3);return t;});
    private ScheduledFuture<?> task;

    public LightFinder() { super("LightFinder","Finds artificially lit areas underground",Category.DONUT); }

    @Override public void onEnable(){results.set(Collections.emptyList());task=exec.scheduleAtFixedRate(this::scan,0,2,TimeUnit.SECONDS);ZRTClient.eventBus.subscribe(Events.RenderWorldEvent.class,e->render(e));}
    @Override public void onDisable(){if(task!=null){task.cancel(false);task=null;}results.set(Collections.emptyList());}
            ...
            for(int cx=-range;cx<=range;cx++) for(int cz=-range;cz<=range;cz++){
                ...
                for(int x=0;x<16;x++) for(int z=0;z<16;z++) for(int y=yMin;y<=yMax;y++){
                    ...
                    if(b==Blocks.AIR||b==Blocks.CAVE_AIR){
                        ...
                        if(lvl>=minLvl){BlockPos cell=new BlockPos((pos.getX()>>4)<<4,(pos.getY()>>4)<<4,(pos.getZ()>>4)<<4);cells.computeIfAbsent(cell,k->new ArrayList<>()).add(pos);}
                    }
                }
            }
            List<LightCluster> found=new ArrayList<>();
            for(Map.Entry<BlockPos,List<BlockPos>> e:cells.entrySet()){
                if(e.getValue().size()>=minCnt){double ax=0,ay=0,az=0;for(BlockPos p:e.getValue()){ax+=p.getX();ay+=p.getY();az+=p.getZ();}int n=e.getValue().size();found.add(new LightCluster(new Vec3d(ax/n,ay/n,az/n),n));}
            }
    ...
    private void render(Events.RenderWorldEvent e){
        ...
        for(LightCluster c:results.get()){Color col=new Color(255,255,0,180);if(showLines.get())RenderUtil.drawLineToPos(e.matrices,mc.player,c.center,col);RenderUtil.drawLabel(e.matrices,c.center,"§eLightCluster §7("+c.count+")",e.tickDelta);}
    }

    public static class LightCluster{public final Vec3d center;public final int count;public LightCluster(Vec3d c,int n){center=c;count=n;}}
}
