...
public class TunnelBaseFinder extends Module {
    ...
    private final ScheduledExecutorService exec = Executors.newSingleThreadScheduledExecutor(r -> {
        Thread t = new Thread(r,"ZRT-TunnelFinder"); t.setDaemon(true); t.setPriority(3); return t;
    });
    ...
    @Override public void onEnable() {
        ...
        ZRTClient.eventBus.subscribe(Events.RenderWorldEvent.class, e -> render(e));
    }
            ...
            for (int cx=-range;cx<=range;cx++) for (int cz=-range;cz<=range;cz++) {
                ...
                for (int y=yMin;y<=yMax;y++) for (int z=0;z<16;z++) {
                    ...
                    for (int x=0;x<16;x++) {
                        ...
                        else{if(run>=ml){Vec3d cen=new Vec3d(chunk.getPos().getStartX()+startX+run/2.0,y+1,chunk.getPos().getStartZ()+z);found.add(new TunnelResult(cen,run,y));}run=0;startX=-1;}
                    }
                }
            }
    ...
    private List<TunnelResult> dedup(List<TunnelResult> list,double d){
        ...
        for(TunnelResult r:list){boolean c=false;for(TunnelResult o:out)if(r.center.distanceTo(o.center)<d){c=true;break;}if(!c)out.add(r);}
        return out;
    }
    ...
    private void render(Events.RenderWorldEvent e) {
        ...
        for (TunnelResult t:results.get()){
            Color col=new Color(255,140,0,180);
            if(showLines.get()) RenderUtil.drawLineToPos(e.matrices,mc.player,t.center,col);
            RenderUtil.drawLabel(e.matrices,t.center,"§6Tunnel §7y="+t.y+" len="+t.length,e.tickDelta);
        }
    }

    public static class TunnelResult { public final Vec3d center; public final int length,y;
