...
public class FreeCam extends Module {
    ...
    // Current and previous position (for smooth lerp interpolation like Astralux)
    ...
    public FreeCam() {
        super("FreeCam", "Smooth detached camera (Astralux style)", Category.MISC, GLFW.GLFW_KEY_UNKNOWN);
    }
    ...
    public void onEnable() {
        ...
        // Init from current camera position
        ...
        ZRTClient.eventBus.subscribe(Events.TickEvent.class, e -> tick());

        // Mouse: rotate camera
        ZRTClient.eventBus.subscribe(Events.MouseMoveEvent.class, e -> {
            if (!isEnabled()) return;
            prevYaw = yaw; prevPitch = pitch;
            yaw   += (float) e.deltaX;
    ...
    // Interpolated values for camera mixin
